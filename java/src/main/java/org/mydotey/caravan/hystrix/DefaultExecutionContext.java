...
import org.mydotey.caravan.hystrix.facade.ExecutionExceptons;
...
public class DefaultExecutionContext implements ExecutionContext {
    ...
    public void markSuccess() {
        ...
        String errorMessage = String.format("Command execution is too long. Command: %s, Execution Time: %s",
            _command.commandKey(), executionLatency);
    ...
    private StartCheckResult canStartExecution() {
        if (!_started.compareAndSet(false, true)) {
            String errorMessage = String.format("Command has been started or run over. Command: %s",
                _command.commandKey());
            _logger.warn(errorMessage);
    ...
    private boolean canEndExecution() {
        ...
        if (!_markedResult.get()) {
            String errorMessage = String.format("Command has not been marked a result. Command: %s",
                _command.commandKey());
            _logger.warn(errorMessage);
        }
